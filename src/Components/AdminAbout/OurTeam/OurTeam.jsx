...
function OurTeam() {
  ...
  const deletefn = async (id) => {
    await axios.get((`${baseURL}/admin/about/deletemember/${id}`))
      .then((response) => {
  ...
  const statusfn = async (id) => {
    await axios.post((`${baseURL}/admin/about/statusmember/${id}`))
      .then((response) => {
  ...
  const fetchteams =  (page,term) => {
        
    axios.get(`${baseURL}/admin/about/showmembers?page=${page}&pageSize=3&search=${term}`)
    .then(response=>{
       const {teams,totalPages} = response.data;
        setTeams(teams);
...
useEffect(() => {
  fetchteams(currentPage,searchItem); 
...
const items = [];
  ...
  let num = 1
  for (const datas of teams) {

      items.push(<tr key={datas.id} >
          <td>{num++}</td>
          <td>{datas.name}</td>
          <td>{datas.designation}</td>
          <td>{<img src={process.env.REACT_APP_PUBLIC_TEAM+datas.image} height={80} alt="team.jpg"/>}</td>
          <td>{datas.status == 1 ? 'Unlocked' : datas.status == 0 ? 'Locked' : null}</td>
          <td>
           <Link to={`/editteam/${datas.id}`}><FontAwesomeIcon title="Edit" icon="fa-solid fa-pen-to-square" /></Link> &nbsp;
            ...
            <div className="row mb-2">
              ...
              <div className="col-sm-6">
                <ol className="breadcrumb float-sm-right">
                  <li className="breadcrumb-item"><a href="/link">Team Members</a></li>
              ...
              <div className="col-12">
                ...
                <div className="card">
                  <div className="card-header">
                    <h3 className="card-title">Team Members Records</h3>
                    <Link className='btn btn-info'style={{ float: 'right' }} to="/addmembers">Add Members</Link>
                  </div>
                  <form>
